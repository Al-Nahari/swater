  ...
  return (
    <motion.header
      ...
      className={`sticky top-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'bg-paper/80 backdrop-blur-xl shadow-[0_1px_0_0_var(--border),0_12px_30px_-18px_rgb(107,76,224,0.35)] py-2'
        ...
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group" onClick={() => setIsMenuOpen(false)}>
            <div className="relative w-11 h-11 rounded-2xl bg-gradient-sun flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform duration-300">
              <span className="text-white font-display font-black text-xl">س</span>
            </div>
          ...
          <nav className="hidden md:flex items-center gap-7">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-ink-soft hover:text-indigo transition-colors font-medium text-[15px] relative group py-1"
              >
                {link.label}
          ...
          <div className="hidden md:flex items-center gap-3">
            <a
              ...
              className="flex items-center gap-2 rounded-full border border-border text-ink px-5 py-2.5 hover:border-indigo hover:text-indigo transition-colors duration-200"
            >
            ...
            <a
              ...
              className="flex items-center gap-2 rounded-full bg-gradient-sun text-white px-5 py-2.5 shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200"
            >
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
              ...
              <nav className="flex flex-col gap-1 py-6 border-t border-border mt-4">
                {NAV_LINKS.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="text-ink-soft hover:text-indigo transition-colors font-medium py-3 px-4 rounded-xl hover:bg-muted"
                ...
                <div className="flex gap-3 pt-4 mt-2">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 rounded-2xl bg-gradient-sun text-white text-center py-3 font-medium"
                  >
                    واتساب
                  </a>
                </div>
