...
import { addServiceImage, deleteServiceImage } from '@/features/services/actions';
import type { ServiceImage } from '@prisma/client';

export default function ServiceImageManager({ serviceId, images }: { serviceId: string; images: ServiceImage[] }) {
  ...
  function handleUpload(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await addServiceImage(serviceId, formData);
      if (!result.success) {
  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-white p-5">
      <h2 className="font-semibold text-neutral-900 mb-4">صور الخدمة</h2>
      ...
      {images.length === 0 ? (
        <p className="text-sm text-neutral-500">لا توجد صور بعد.</p>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {images.map((img) => (
            <div key={img.id} className="relative rounded-lg overflow-hidden border border-neutral-200">
              ...
              {img.isCover && (
                <span className="absolute top-1 right-1 bg-neutral-900 text-white text-[10px] px-1.5 py-0.5 rounded">
                  رئيسية
                </span>
              )}
              <div className="p-2">
  ...
  return (
    <button
      ...
      onClick={() => {
        ...
        startTransition(() => { void deleteServiceImage(imageId); });
      }}
